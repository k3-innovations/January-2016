...
Delay_Relation = foreach Airport_Relation  generate  Month as mon , DepDelay as delay;

Group_delay = group Delay_Relation by  mon;


Count_delay = FOREACH Group_delay {
			Filter_delay = FILTER Delay_Relation BY (delay >= 15); 
			GENERATE group,  (double)COUNT(Filter_delay)/COUNT(Delay_Relation) AS fracmon;
			};
Order_delay= order Count_delay by fracmon desc;
