...
Delay_Relation = foreach Airport_Relation  generate  Year as year, DepDelay as delay;

Group_delay = group Delay_Relation by year;


Count_delay = FOREACH Group_delay {
			Filter_delay = FILTER Delay_Relation BY (delay >= 15); 
			GENERATE group,  (double)COUNT(Filter_delay)/COUNT(Delay_Relation) AS fracyr;
			};
Order_delay= order Count_delay by fracyrdesc;
